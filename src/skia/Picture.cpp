...
py::class_<SkRTreeFactory, SkBBHFactory>(m, "RTreeFactory");
...
auto picture = static_cast<py::class_<SkPicture, PyPicture, sk_sp<SkPicture>,
    SkRefCnt>>(m.attr("Picture"));
picture
...
auto drawable = static_cast<py::class_<SkDrawable, sk_sp<SkDrawable>,
    SkRefCnt>>(m.attr("Drawable"));
drawable
    .def("draw",
...
auto rtreefactory = static_cast<py::class_<SkRTreeFactory, SkBBHFactory>>(
    m.attr("RTreeFactory"));
rtreefactory
    .def(py::init<>())
    .def("__call__", &SkBBHFactory::operator());

auto bboxhierarchy = static_cast<py::class_<SkBBoxHierarchy, PyBBoxHierarchy,
    sk_sp<SkBBoxHierarchy>, SkRefCnt>>(m.attr("BBoxHierarchy"));

auto metadata = static_cast<py::class_<SkBBoxHierarchy::Metadata>>(
    bboxhierarchy.attr("Metadata"));
metadata
...
auto picturerecorder = static_cast<py::class_<SkPictureRecorder>>(
    m.attr("PictureRecorder"));
picturerecorder
    .def(py::init())
    // .def("beginRecording",
